...
import { bandByKey, type BillBandKey } from '@/lib/qualifying';
...
export async function submitLead(formData: FormData) {
  const billBandRaw = String(formData.get('bill_band') ?? '').trim();
  const billBand: BillBandKey | null = bandByKey(billBandRaw)?.key ?? null;
  ...
  // Same neutral thank-you for EVERY successful capture — qualified,
  // disqualified, or uncovered. The homeowner is never told they were
  // rejected; the owner sees the truth on the Leads screen.
